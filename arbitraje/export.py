...
def _sanitize(v):
    """Neutraliza fórmulas Excel: = + - @ | % al inicio."""
    s = str(v)
...
def _export_csv(conn, directorio):
    ...
    with open(ruta_jornadas, "w", newline="", encoding="utf-8") as archivo:
        ...
        for j in repo.listar_jornadas(conn):
            escritor.writerow([
                _sanitize(j["id"]), _sanitize(j["fecha"]), _sanitize(j["estado"]),
                _sanitize(j["partidos_total"] if j["partidos_total"] is not None else ""),
                _sanitize(j["roles_detalle"] or ""),
    ...
    with open(ruta_descuentos, "w", newline="", encoding="utf-8") as archivo:
        ...
        for f in filas:
            escritor.writerow([_sanitize(f["id"]), _sanitize(f["fecha"]), _sanitize(f["concepto"]), _sanitize(f["monto"]), _sanitize(f["nota"] or "")])
    _chmod_600(ruta_descuentos)
...
def _export_xls(conn, directorio):
    ...
    with open(ruta, "w", encoding="utf-8") as f:
        ...
        for r in descuentos_rows:
            f.write(f"<tr><td>{html.escape(_sanitize(r['fecha']))}</td><td>{html.escape(_sanitize(r['concepto']))}</td><td>{html.escape(_sanitize(r['monto']))}</td><td>{html.escape(_sanitize(r['id']))}</td></tr>")
    ...
    def row_xml(cells, row_num):
        ...
        for col_idx, val in enumerate(cells, 1):
            ...
            if isinstance(val, (int,float)) and val != "":
                # números: verificar que no sea fórmula disfrazada (ya sanitizado pero por si)
